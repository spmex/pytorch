...
template <>
inline void convertImpl<float, int8_t>(
    const float* __restrict src,
    int8_t* __restrict dst,
    int64_t n) {
  uint64_t len = static_cast<uint64_t>(n);
  for (uint64_t i = 0; i < len; i++) {
    dst[i] = static_cast<int8_t>(std::clamp(
...
template <int src_n>
inline int8x16_t convert_float_to_int8(const VectorizedN<float, src_n>& src) {
  const auto to_i16 = [](const float32x4_t lo, const float32x4_t hi) {
    return vcombine_s16(vqmovn_s32(vcvtq_s32_f32(lo)), vqmovn_s32(vcvtq_s32_f32(hi)));
  };
  ...
  if constexpr (src_n == 4) {
    return vcombine_s8(
        vqmovn_s16(to_i16(src[0], src[1])), vqmovn_s16(to_i16(src[2], src[3])));
  } else if constexpr (src_n >= 2) {
    return vcombine_s8(vqmovn_s16(to_i16(src[0], src[1])), zero);
  } else {
    return vcombine_s8(
        vqmovn_s16(to_i16(src[0], vdupq_n_f32(0.0f))), zero);
  }
}

template <int src_n>
struct VecConvert<int8_t, 1, float, src_n> {
  static inline VectorizedN<int8_t, 1> apply(
      const VectorizedN<float, src_n>& src) {
    return Vectorized<int8_t>(convert_float_to_int8(src));
  }
};
